...
export async function oilExplorationOverlap(
  sketch: Sketch<Polygon> | SketchCollection<Polygon>
): Promise<ReportResult> {
  const box = sketch.bbox || bbox(sketch);
  const metricGroup = project.getMetricGroup("oilExplorationOverlap");
...
export default new GeoprocessingHandler(oilExplorationOverlap, {
  title: "oilExplorationOverlap",
  description:
    "Calculate sketch overlap with oil and gas exploration area polygons",
  executionMode: "async",
  timeout: 600,
  requiresProperties: [],
});
